...
class BaseAcceptanceMapCreator(ABC):
    def __init__(
        ...
        self.interpolation_cleaning_energy_relative_threshold = (
            interpolation_cleaning_energy_relative_threshold
        )
        self.interpolation_cleaning_spatial_relative_threshold = (
            interpolation_cleaning_spatial_relative_threshold
        )
    ...
    def _transform_obs_to_camera_frame(
        ...
        rotate_to_obs : gammapy.data.observations.Observation
            All events of `obs` are rotate in camera coordinates to match
            the azimuth angle of `rotate_to_obs`.
        ...
        pointing_altaz = obs.get_pointing_icrs(obs.events.time).transform_to(
            altaz_frame
        )
        ...
        camera_frame_obs_info["RA_PNT"] = 0.0
        camera_frame_obs_info["DEC_PNT"] = 0.0
    ...
    def _transform_exclusion_region_to_camera_frame(
        ...
        for region in self.exclude_regions:
            if isinstance(region, CircleSkyRegion):
                ...
                width_coordinate = center_coordinate.directional_offset_by(
                    region.angle, region.width
                )
                width_coordinate_altaz = width_coordinate.transform_to(pointing_altaz)
                width_coordinate_camera_frame = width_coordinate_altaz.transform_to(
                    camera_frame
                )
                angle_camera_frame = center_coordinate_camera_frame.position_angle(
                    width_coordinate_camera_frame
                ).to(u.deg)[0]
                center_coordinate_camera_frame_arb = SkyCoord(
                    ra=center_coordinate_camera_frame.lon[0],
                    dec=center_coordinate_camera_frame.lat[0],
                ...
                exclude_region_camera_frame.append(
                    EllipseSkyRegion(
                        center=center_coordinate_camera_frame_arb,
                        width=region.width,
    ...
    def _create_map(
        ...
        maker_safe_mask = SafeMaskMaker(
            methods=["offset-max"], offset_max=self.max_offset
        )
    ...
    def _compute_time_intervals_based_on_fov_rotation(self, obs: Observation) -> Time:
        ...
        n_bin = max(
            ...
            int(
                np.rint(
                    ((obs.tstop - obs.tstart) / self.time_resolution).to_value(
    ...
    def _normalised_model_per_run(
        self, observations: Observations, acceptance_map: dict[int, BackgroundIRF]
        ...
        for obs in observations:
            ...
            if norm_background < 0.0:
                logger.error(
                    ...
                    + " : "
                    + str(norm_background)
                    + ", normalisation set back to 1"
                )
                norm_background = 1.0
            elif norm_background > 1.5 or norm_background < 0.5:
                logger.warning(
                    "High correction of the background normalisation for run "
                    + str(id_observation)
                    + " : "
                    + str(norm_background)
                )
    ...
    def _compute_time_intervals_based_on_zenith_bin(
        self, obs: Observation, edge_zenith_bin: u.Quantity
    ) -> Time:
        """
        ...
        with erfa_astrom.set(ErfaAstromInterpolator(1000 * u.s)):
            ...
            if np.any(zenith_values < np.min(edge_zenith_bin)) or np.any(
                zenith_values > np.max(edge_zenith_bin)
            ):
                logger.error(
                    "Run with zenith value outside of the considered range for zenith binning"
                )
    ...
    def create_model_cos_zenith_binned(
        ...
        try:
            ...
            logger.error(
                f" KeyError : {self.cos_zenith_binning_method} not a valid zenith binning method.\nValid "
        ...
        if self.zenith_binning_run_splitting:
            if abs(i_method) == 2:
                logger.warning(
                    "Using a zenith binning requirement based on n_observation while using run splitting is not recommended and could lead to poor models. We recommend switching to a binning requirement based on livetime."
                )
            compute_observations = Observations()
            for obs in observations:
                time_interval = self._compute_time_intervals_based_on_zenith_bin(
                    obs, zenith_bin
                )
                for i in range(len(time_interval) - 1):
                    compute_observations.append(
                        obs.select_time(Time([time_interval[i], time_interval[i + 1]]))
                    )
        else:
            compute_observations = observations
        ...
        livetime_observations = np.array(
            [
                obs.observation_live_time_duration.to_value(u.s)
                for obs in compute_observations
            ]
        )
        ...
        if per_wobble:
            wobble_observations = np.array(
                get_unique_wobble_pointings(
                    compute_observations, self.max_angular_separation_wobble
                )
            )
            multiplicity_wob = 1
        else:
            wobble_observations = np.full(
                len(cos_zenith_observations), "any", dtype=np.object_
            )
            multiplicity_wob = 0
        ...
        for wobble in np.unique(wobble_observations):
            ...
            cumsum_variable[wobble] = np.cumsum(
                np.histogram(
                    cos_zenith_observations[wobble_observations == wobble],
                    bins=cos_zenith_bin,
                    weights=cut_variable_weights[wobble_observations == wobble],
                )[0]
            )
        ...
        while i < n:
            ...
            candidate_i_per_wobble = [
                np.nonzero(cum_cut_variable >= self.cos_zenith_binning_parameter_value)[
                    0
                ][:1]
                for cum_cut_variable in cumsum_variable.values()
            ]  # nonzero is assumed sorted
        ...
        logger.info(
            f"observation per bin: {list(np.histogram(cos_zenith_observations, bins=cos_zenith_bin)[0])}"
        )
        logger.info(
            f"livetime per bin [s]: "
        ...
        if per_wobble:
            wobble_observations_bool_arr = [
                (np.array(wobble_observations.tolist()) == wobble)
                for wobble in np.unique(np.array(wobble_observations))
            ]
            livetime_observations_and_wobble = [
                np.array(livetime_observations) * wobble_bool
                for wobble_bool in wobble_observations_bool_arr
            ]
        ...
        for i in range(len(binned_model)):
            collection_binned_model[np.rad2deg(np.arccos(bin_center[i]))] = (
                binned_model[i]
            )
    ...
    def create_acceptance_map_cos_zenith_binned(
        ...
        if off_observations is None:
            ...
            logger.warning(
                "The off observations provided will be ignored as a base model has been provided."
            )

        # If needed produce the zenith binned model
        if base_model is not None and not isinstance(
            base_model, BackgroundCollectionZenith
        ):
            error_message = (
                "The models should be provided as a BackgroundCollectionZenith object"
            )
            logger.error(error_message)
            raise BackgroundModelFormatException(error_message)
        dict_binned_model = base_model or self.create_model_cos_zenith_binned(
            off_observations
        )
        ...
        for obs in observations:
            if self.use_mini_irf_computation:
                evaluation_time, observation_time = get_time_mini_irf(
                    obs, self.mini_irf_time_resolution
                )
                ...
                data_obs_all = (
                    np.zeros(
                        tuple(
                            ...
                            + list(dict_binned_model[key_model[0]].data.shape)
                        )
                    )
                    * dict_binned_model[key_model[0]].unit
                )
                for i in range(len(evaluation_time)):
                    cos_zenith_observation = np.cos(
                        obs.get_pointing_altaz(evaluation_time[i]).zen
                    )
                    key_closest_model = key_model[
                        (np.abs(cos_zenith_model - cos_zenith_observation)).argmin()
                    ]
                ...
                if type_model is Background2D:
                    acceptance_map[obs.obs_id] = Background2D(
                        axes=axes_model, data=data_obs
                    )
                elif type_model is Background3D:
                    acceptance_map[obs.obs_id] = Background3D(
                        axes=axes_model, data=data_obs, fov_alignment=FoVAlignment.ALTAZ
                    )
                else:
    ...
    def _create_interpolation_function(
        self, base_model: BackgroundCollectionZenith
    ) -> interp1d:
        """
        ...
        if self.interpolation_type == "log":
            interp_func = interp1d(
                x=cos_zenith_model,
                y=np.log10(
                    data_cube.to_value(binned_model[0].unit)
                    + self.threshold_value_log_interpolation
                ),
                axis=0,
    ...
    def _background_cleaning(self, background_model):
        ...
        i = 0
        while (i < 1 or not np.allclose(base_model, final_model)) and (
            i < self.max_cleaning_iteration
        ):
            base_model = final_model.copy()
            i += 1

            count_valid_neighbour_condition_energy = compute_neighbour_condition_validation(
                base_model,
                axis=0,
                relative_threshold=self.interpolation_cleaning_energy_relative_threshold,
            )
            count_valid_neighbour_condition_spatial = compute_neighbour_condition_validation(
                base_model,
                axis=1,
            ...
            mask_spatial = count_valid_neighbour_condition_spatial > (
                1 if base_model.ndim == 3 else 0
            )
            mask_valid = np.logical_and(mask_energy, mask_spatial)
            final_model[~mask_valid] = 0.0

        return final_model

    def _get_interpolated_background(
        self, interp_func: interp1d, zenith: u.Quantity
    ) -> np.array:
        """
        Create the function that will perform the interpolation
        ...
        if self.interpolation_type == "log":
            interp_bkg = 10.0 ** interp_func(np.cos(zenith))
            interp_bkg[interp_bkg < 100 * self.threshold_value_log_interpolation] = 0.0
        elif self.interpolation_type == "linear":
    ...
    def create_acceptance_map_cos_zenith_interpolated(
        self,
        observations: Observations,
        off_observations: Observations = None,
        base_model: BackgroundCollectionZenith = None,
        ...
        if len(dict_binned_model) <= 1:
            ...
            for obs in observations:
                acceptance_map[obs.obs_id] = dict_binned_model[
                    dict_binned_model.zenith[0]
                ]
        else:
            ...
            for obs in observations:
                if self.use_mini_irf_computation:
                    ...
                    for i in range(len(evaluation_time)):
                        data_obs_bin = self._get_interpolated_background(
                            interp_func, obs.get_pointing_altaz(evaluation_time[i]).zen
                        )
                        data_obs_all[i, :, :] = data_obs_bin

                    data_obs = generate_irf_from_mini_irf(
                        data_obs_all, observation_time
                    )

                else:
                    data_obs = self._get_interpolated_background(
                        interp_func, obs.get_pointing_altaz(obs.tmid).zen
                    )

                if type_model is Background2D:
                    acceptance_map[obs.obs_id] = Background2D(
                        axes=axes_model, data=data_obs * unit_model
                    )
                elif type_model is Background3D:
                    acceptance_map[obs.obs_id] = Background3D(
                        axes=axes_model,
    ...
    def create_acceptance_map_per_observation(
        ...
        if zenith_interpolation:
            acceptance_map = self.create_acceptance_map_cos_zenith_interpolated(
                observations=observations,
                off_observations=off_observations,
                base_model=base_model,
            )
        elif zenith_binning:
            acceptance_map = self.create_acceptance_map_cos_zenith_binned(
                observations=observations,
                off_observations=off_observations,
                base_model=base_model,
            )
            ...
            if base_model is not None:
                if not isinstance(base_model, BackgroundIRF):
                    error_message = (
                        "The model provided should be a gammapy BackgroundIRF object"
                    )
                    logger.error(error_message)
                    raise BackgroundModelFormatException(error_message)
                unique_base_acceptance_map = base_model
            else:
                unique_base_acceptance_map = self.create_acceptance_map(
                    off_observations
                )
            for obs in observations:
