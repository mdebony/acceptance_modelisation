...
class BaseAcceptanceMapCreator(ABC):
    ...
    def _create_model_cos_zenith_binned(self,
        ...
        if self.zenith_binning_run_splitting:
            if abs(i_method) == 2:
                logger.warning('Using a zenith binning requirement n_observation at the same time of using run splitting is not recommanded and could lead to poor model. We recommand switching to a binning requirement based on livetime.')
